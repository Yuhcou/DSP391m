...
def random_eval(env, n_episodes=5):
    returns = []
    waiting = []
    incar = []
...
def trained_eval(env, config_path, n_episodes=5):
...
if __name__ == "__main__":
    config_path = os.path.join('dsp2', 'configs', 'improved.yaml')
    cfg = load_config(config_path)
    env = make_env(cfg)

    print("=" * 80)
    print("AGENT COMPARISON - IMPROVED CONFIG")
    print("=" * 80)
    print(f"\nEnvironment: lambda={cfg['lambda']}, t_max={cfg['t_max']}s")
    print(f"             {cfg['n_floors']} floors, {cfg['m_elevators']} elevators\n")

    print("Evaluating RANDOM policy...")
    random_results = random_eval(env, n_episodes=10)
    ...
    trained_results = trained_eval(env, config_path, n_episodes=10)
    if trained_results:
        ...
        improvement = ((trained_results['mean_return'] - random_results['mean_return']) /
                      abs(random_results['mean_return']) * 100)
        print(f"Return Improvement:  {improvement:+.1f}%")

        wait_improvement = ((random_results['mean_waiting'] - trained_results['mean_waiting']) /
                           random_results['mean_waiting'] * 100)
        print(f"Waiting Reduction:   {wait_improvement:+.1f}%")
